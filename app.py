...
st.caption("1990–2023 지진 데이터 기반 K-Means 군집 분석 (2022–2023, 500개 샘플)")
...
@st.cache_data
def load_sample():
    return joblib.load("earthquake-sample.pkl")
...
with tab2:
    ...
    for c, color in color_dict.items():
        sub = df_sample[df_sample['cluster'] == c]
        ax.scatter(sub['진원깊이'], sub['규모'], c=color, label=f"군집{c}({risk_dict[c]})", alpha=0.6, s=20)
    ax.set_xlabel("진원깊이")
    ax.set_ylabel("규모")
    ax.set_title("세계 지진 분석")
    ax.legend()
...
with tab3:
    ...
    for c, color in color_dict.items():
        sub = df_sample[df_sample['cluster'] == c]
        ax.scatter(sub['쓰나미여부'], sub['규모'], c=color, label=f"군집{c}({risk_dict[c]})", alpha=0.6, s=20)
    ax.set_xlabel("쓰나미 여부")
    ax.set_ylabel("규모")
    ax.set_title("세계 지진 분석")
    ax.legend()
